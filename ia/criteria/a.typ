...
Fitting a set amount of tasks into limited free time around fixed events is a combinatorics problem. The number of possible arrangements grows rapidly, and each combination has to be validated against several constraints at once (overlaps, deadlines, daily capacity). This is exactly the repetitive rule based process a computer performs quickly and consistently. Meanwhile I, the human, do it slowly and make mistakes. Therefore the problem requires an algorithm.

I will build a terminal program in *Python 3, standard library only*: `datetime` for deadline arithmetic on 15-minute blocks, and `json` for human-readable persistence. Python is fully suitable for this task without adding unnecessary complexities or dependencies, and it's a language I've been learning and can explain line by line. I also looked at existing tools: *Google Calendar* stores both events and deadlines, but it can't place them on the same canvas, and doens't prevent conflicts. *Todoist* ranks a to-do list by date and priority but never checks whether the work actually _fits_ before its deadline, so infeasibility goes undetected. *Morgen* is too expensive for me as a student, too manual because I still need to declare free slots, and it deosn't allow me switch between algorithms.

== Success criteria

The solution is successful if:

+ Tasks (name, duration, deadline, priority) and events (name, start, end, repetition) can be added, edited, and deleted via a menu.
...
+ A task longer than one free gap is split into smaller ones, possibly across days, with each part showing progress (e.g. `1h30 of 4h00`).
